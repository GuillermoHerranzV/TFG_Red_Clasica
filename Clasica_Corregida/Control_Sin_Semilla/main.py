...
def run():
    ...
    with config.CSV_PATH.open("a", newline="", encoding="utf-8") as f:
        ...
        for use_reduction in config.USE_REDUCTION_OPTIONS:
            latent_dim_values = config.LATENT_DIMS if use_reduction else [None]

            for latent_dim in latent_dim_values:
                print("\n====================================================")
                print(f"use_reduction = {use_reduction} | latent_dim = {latent_dim}")
                print("====================================================")
                ...
                if use_reduction:
                    autoencoder, encoder = build_conv_autoencoder(
                        ...
                        target_hw=config.TARGET_HW[latent_dim],
